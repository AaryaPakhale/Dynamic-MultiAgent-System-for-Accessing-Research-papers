...
class BaseAnalysisAgent:
    ...
    def __init__(self, section_type):
        ...
        self.llm = ChatTogether(
            ...
            timeout=None,
            max_retries=2,
            api_key = "6a116c560f7e7d3775a9e56683bfebf478085f97136ac0c572490e2867756bec"
        )
...
class DecisionMakingAgent:
    ...
    def __init__(self):
        ...
        self.decision_prompt = ChatPromptTemplate.from_messages([
            ...
            Your role is to provide detailed, actionable feedback on papers while determining
            their publication suitability. Classify papers as either "Publishable" or 
            "Non-Publishable" based on these specific criteria:

            Critical Issues Requiring Specific Feedback:
            1. Methodology Problems:
               - Statistical tests: Identify misapplied tests, suggest correct alternatives
               - Sample size: Specify if underpowered, calculate required sample size
               - Control groups: Note missing controls, suggest specific controls needed
               - Validation: List missing validation steps, recommend specific techniques
               
            2. Argument Structure Issues:
               - Literature gaps: Identify specific missing citations or perspectives
               - Logical flow: Pinpoint exact locations of disconnects between sections
               - Claims hierarchy: Map which supporting claims are missing/weak
               - Counter-arguments: List unaddressed alternative explanations
               
            3. Evidence Quality Issues:
               - Data presentation: Specify missing error bars, statistical tests, or controls
               - Result validation: Note which results need independent replication
               - Benchmark comparison: List relevant benchmarks missing from evaluation
               - Limitation analysis: Identify specific limitations needing discussion
               
            Required Feedback Components:
            1. Location Reference: Cite specific sections, paragraphs, or figures
            2. Problem Description: Describe the exact issue found
            3. Impact Assessment: Explain how it affects paper validity
            4. Suggested Fix: Provide actionable steps to address the issue
            
            Evaluation Process:
            1) Thought: Review each section with above criteria checklist
            2) Action: Document specific issues found with exact locations
            3) Observation: Analyze severity and interdependence of issues
            4) Thought: Determine if issues are fixable within revision
            5) Action: Provide detailed feedback organized by category
            
            Previous similar papers showed these patterns:
            {patterns}
            
            Format your response as:
            Thought: [Detailed section-by-section review]
            Action: [Specific issue identification with locations]
            Observation: [Issue severity analysis]
            Thought: [Revision feasibility assessment]
            Action: [Categorized feedback]
            ...
            POSITIVE ASPECTS:
            - [List specific strengths with locations]
            
            REASONING: [Integration of issues and strengths into final decision]"""),
...
def create_evaluation_workflow(memory_manager: MemoryManager):
    """Creates the evaluation workflow graph with parallel section analysis"""
    ...
    def analyze_section(args):
        """Helper function for parallel section analysis"""
        section_type, agent, content, patterns = args
        analysis = agent.analyze(content, patterns)
        return section_type, analysis
...
def evaluate_papers(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for idx, row in df.iterrows():
        ...
        papers_queue.append(PaperState(paper_id=str(df['filename'].iloc[idx]), features=features))
    print("Extracted from dataframe")
    ...
    # Run evaluation
    final_state = workflow.invoke(initial_state, config={"recursion_limit": 150})
...
if __name__ == "__main__":
    # Evaluate papers
    df = pd.read_csv('/Users/avinandan/Desktop/KDSH Task 1/paper_df_leftover.csv')
    print(df.tail())
    results_df_test = evaluate_papers(df)
    results_df_test.to_csv('results_df_test_leftovers.csv', index=False)
